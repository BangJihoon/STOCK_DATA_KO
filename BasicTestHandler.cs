...
namespace YOACOMClientCSharp
{
    class BasicTestHandler
    {
        ...
        public void Search()
        {
            ...
            //추가부분 
            string[] jongCodeList = new string[] { "005930", "105560", "207940", "180640" };
            

            foreach (string i in jongCodeList)
            {
                int nErr = m_iYuantaAPI.YOA_SetTRInfo("300001", "InBlock1");
                //추가부분 
                //MessageBox.Show("nErr: " + nErr);       // 202 DSO를 찾을수없음
                ...
                nErr = m_iYuantaAPI.YOA_SetFieldString("jongcode", i, 0);   //m_strJongCode를 i로 수정함
                if (CommDef.ERROR_MAX_CODE > nErr)
                {
                    string strMsg = m_iYuantaAPI.YOA_GetErrorMessage(nErr);     // 조회 요청 실패에 대한 메시지를 얻을 수 있습니다.
                    m_mainForm.lbLog.Items.Insert(0, strMsg);
                ...
                //추가부분 
                //MessageBox.Show("nReqID : " + nReqID);  // -1 알수없는 오류
        ...
        public void RegistAuto()
        {
            ...
            m_iYuantaAPI.YOA_SetTRFieldString("11", "InBlock1", "jongcode", m_mainForm.txtBTJongCode.Text, 0);      //m_mainForm.txtBTJongCode.Text를 i로 바꿈
        ...
        public void ReceiveData(int nReqID)
        {
            ...
            if (-1 != strTRID.CompareTo("300001"))
            {
                ...
                // 추가부분 - 메인 로그에 정보찍기
                m_mainForm.LogMessage("\t종목명 :" + m_iYuantaAPI.YOA_GetTRFieldString("300001", "OutBlock1", "jongname", 0));       // 종목명
                m_mainForm.LogMessage("\t현재가 :" + m_iYuantaAPI.YOA_GetTRFieldString("300001", "OutBlock1", "curjuka", 0));       // 현재가
                m_mainForm.LogMessage("\t전일대비 :" + m_iYuantaAPI.YOA_GetFieldString("debi", 0));      // 전일대비
                m_mainForm.LogMessage("\t등락률 :" + m_iYuantaAPI.YOA_GetFieldString("debirate", 0));    // 등락률
                m_mainForm.LogMessage("\t거래량 :" + m_iYuantaAPI.YOA_GetFieldString("volume", 0));      // 거래량
                m_mainForm.LogMessage("------------------------------------------");                     // 구분선
            }
        }
        ...
        public void ReceiveRealData(int nReqID, string strAutoID)
        {
            if (-1 != strAutoID.CompareTo("11"))
            {
                ...
                if (m_strJongCode.Equals(strOutCode))
                {
                    ...
                    /*
                    // 추가부분 - 메인 로그에 정보찍기
                    m_mainForm.LogMessage("\t종목명 :" + m_iYuantaAPI.YOA_GetTRFieldString("11", "OutBlock1", "jongcode", 0));       // 종목명
                    m_mainForm.LogMessage("\t현재가 :" + m_iYuantaAPI.YOA_GetTRFieldString("11", "OutBlock1", "curjuka", 0));       // 현재가
                    m_mainForm.LogMessage("\t전일대비 :" + m_iYuantaAPI.YOA_GetTRFieldString("11", "OutBlock1", "debi", 0));      // 전일대비
                    m_mainForm.LogMessage("\t등락률 :" + m_iYuantaAPI.YOA_GetTRFieldString("11", "OutBlock1", "debirate", 0));    // 등락률
                    m_mainForm.LogMessage("\t거래량 :" + m_iYuantaAPI.YOA_GetTRFieldString("11", "OutBlock1", "volume", 0));      // 거래량
                    m_mainForm.LogMessage("------------------------------------------");                     // 구분선
                    */
                }
            }
